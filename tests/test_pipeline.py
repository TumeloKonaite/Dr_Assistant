...
def test_run_full_pipeline_writes_outputs_under_output_dir():
    with patch(
        "src.pipelines.run_full_pipeline.Path.exists",
        return_value=True,
    ), patch(
        "src.pipelines.run_full_pipeline.Path.mkdir",
    ), patch(
        "src.pipelines.run_full_pipeline.Path.write_text",
    ) as mock_write_text, patch(
        "src.pipelines.run_full_pipeline._clear_existing_chunks",
    ) as mock_clear, patch(
        "src.pipelines.run_full_pipeline.normalize_audio",
        return_value="data/output/meeting_normalized.wav",
    ) as mock_normalize, patch(
        "src.pipelines.run_full_pipeline.chunk_audio",
    ) as mock_chunk, patch(
        "src.pipelines.run_full_pipeline.run_transcription_pipeline",
        return_value="hello world",
    ) as mock_transcribe:
        result = run_full_pipeline("meeting.mp4", output_dir="data/output")
...
def test_run_full_pipeline_falls_back_to_source_media_on_disk_space_error():
    with patch(
        "src.pipelines.run_full_pipeline.Path.exists",
        return_value=True,
    ), patch(
        ...
        "src.pipelines.run_full_pipeline.normalize_audio",
        side_effect=RuntimeError("FFmpeg normalization failed:\nNo space left on device"),
    ), patch(
        "src.pipelines.run_full_pipeline.chunk_audio",
    ) as mock_chunk, patch(
        "src.pipelines.run_full_pipeline.run_transcription_pipeline",
